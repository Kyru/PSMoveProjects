...
//				else
			...
			case DataStructs.STATUS.STREAMING:
				...
				if (!ignoreTracking && trinusCamera != null)
					...
					// <F>
					trinusCamera.transform.localEulerAngles = new Vector3 ( sensorData.pitch,  0f /* (sensorData.yaw - yawReset) * settingsInstance.yawScale*/, 0f);
				...
				//}
				// <F> 
				cameraRotation.Set (sensorData.quatX, sensorData.quatY, 0f, sensorData.quatW);// * Quaternion.Inverse(test);
				if (!ignoreTracking && trinusCamera != null) {
					trinusCamera.transform.localRotation = cameraRotation;
				}
				#endif
